...
class VGG16BidirectionalLSTMVideoClassifier(object):
    ...
    def create_model(self):
        ...
        model_dir_path = os.path.join(os.path.dirname(__file__), '/home/prathyush/Video-classifier-keras-master/results/models', data_set_name)
    ...
    def fit(self, data_dir_path, model_dir_path, vgg16_include_top=True, data_set_name='UniCrime', test_size=0.2,
            random_state=42):
...
class VGG16LSTMVideoClassifier(object):
    ...
    def create_model(self):
        ...
        #For a ConvLSTM(), the input shape to your neural network is [samples, timesteps, rows, columns, features].
        #samples=30 
       
        model.add(ConvLSTM2D(filters=128, kernel_size=(3,3), activation='relu', input_shape=(8, 30, 7, 7,512), return_sequences = True))
        model.add(ConvLSTM2D(filters=64, kernel_size=(3,3), activation='relu'), return_sequences = False)
        #The return_sequences returns the hidden state output for each input time step. If set to false, in Keras it returns only the last hidden state output. 
        #When using stacked LSTMs(standard LSTMs), you need to set return_sequences to True so that the next LSTM has a 3-D input
        
        #model.add(LSTM(units=HIDDEN_UNITS, input_shape=(None, self.num_input_tokens), return_sequences=False, dropout=0.5))
        #model.add(LSTM(units=HIDDEN_UNITS, input_shape=(None, self.num_input_tokens), return_sequences=False, dropout=0.5))
        #model.add(LSTM(units=HIDDEN_UNITS, input_shape=(None, HIDDEN_UNITS), return_sequences=True, dropout=0.4))
        #model.add(LSTM(units=HIDDEN_UNITS, input_shape=(None, HIDDEN_UNITS), return_sequences=False, dropout=0.3))
    ...
    def fit(self, data_dir_path, model_dir_path, vgg16_include_top=True, data_set_name='UCF-101', test_size=0.2, random_state=42):
        ...
        checkpoint = ModelCheckpoint(filepath=weight_file_path, save_best_only=True)
        history = parallel_model.fit_generator(generator=train_gen, steps_per_epoch=train_num_batches,
                                      epochs=NUM_EPOCHS,
